...
class DataConfig:
    ...
    random_seed: int = 42
    test_size: float = 0.10           # final test fraction (stratified by age)
    val_size: float = 0.15            # validation fraction of the remaining (1 - test_size)
...
class FeatureConfig:
    """
    Exactly four active descriptors (Config B):

    1) ``use_global_freq``  2) ``use_char_count``
    3) ``use_punctuation_type``  4) ``use_pos_tag``

    All other feature flags stay False. ``feature_dim`` == 4.
    Values are z-scored per column with train-only statistics (``build_datasets``).
    """

    # 1. Frequency (corpus-level only among frequency family)
...
class ModelConfig:
    ...
    dropout: float = 0.5            # baseline aligned with ~60% doc run (global norm)

# ── Training config ───────────────────────────────────────────────────────────

@dataclass
class TrainingConfig:
    """Training loop: baseline after global normalization (best known in-repo)."""
    batch_size: int = 32
    learning_rate: float = 1e-3
    weight_decay: float = 5e-4
    label_smoothing: float = 0.0    # CrossEntropyLoss; keep 0 for baseline
    num_epochs: int = 50
